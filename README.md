...
> [!IMPORTANT]
> When using C3 to create this project, select "no" when it asks if you want to deploy. You need to follow this project's [setup steps](https://github.com/cloudflare/templates/tree/main/polished-mud-82c9#setup-steps) before deploying.
...
```bash
npm create cloudflare@latest -- --template=cloudflare/templates/polished-mud-82c9
```

A live public deployment of this template is available at [https://polished-mud-82c9.templates.workers.dev](https://polished-mud-82c9.templates.workers.dev)
...
1. Install the project dependencies with a package manager of your choice:
   ...
   npm install
   ```
2. Create a [D1 database](https://developers.cloudflare.com/d1/get-started/) with the name "white-sound-eaeb-d1":
   ```bash
   npx wrangler d1 create white-sound-eaeb-d1
   ```
   ...and update the `database_id` field in `wrangler.json` with the new database ID.
3. Run the following db migration to initialize the database (notice the `migrations` directory in this project):
   ```bash
   npx wrangler d1 migrations apply --remote white-sound-eaeb-d1
   ```
